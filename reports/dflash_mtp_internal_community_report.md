...
先把 `original MTP`、`original DFlash`、`baseline`、`trained MTP`、`trained DFlash` 放在一张表里。红色行为我们训练或权重插值后的结果；提升比例均相对各自原始方法计算。
...
9B 上有一个很有意思的结论: **trained DFlash 的接受长度仍低于 MTP，但 QPS 最高**。原因是 DFlash draft 更便宜，在 9B 这个规模上，低 draft 成本足以抵消部分接受率差距。
...
122B 上结论反过来: **MTP 同时拿到最高接受长度和最高 QPS**。大模型 verifier 前向代价更高，MTP 高接受长度带来的收益更明显。若只看 ALLaVA 域内最优点，`beta=0.6` 纯微调 MTP 可达到 **36.82 token/s、3.419 mean accept/draft**，相对 native **QPS +7.3%、accept +11.5%**；表中采用 `alpha=0.5 soup` 是因为它同时修复了 MMStar 域外回退，更适合通用服务。

### 0.3 一句话总结

**DFlash 是“相对提升最大”的路线，MTP 是“绝对接受率最高”的路线。**
DFlash 开源 draft 原本没有多模态数据训练，自蒸馏后补齐了图文分布，所以提升显著；MTP 原生头已经有多模态能力，起点更高，因此提升幅度较小，但最终接受长度仍然最高。

## 1. 背景: 投机解码为什么能加速

多模态大模型推理中的主要成本来自逐 token 自回归解码。投机解码(speculative decoding)的核心思想是: 先用一个 draft 机制一次提出多个候选 token，再由主模型 verifier 一次前向并行校验这些候选 token。只要候选 token 与 verifier 的 greedy 输出一致，就可以一次接受多个 token。
...
这里最关键的指标就是 `mean accept/draft`，即每轮投机平均被 verifier 接受多少个 token。它不是 0 到 1 之间的概率，而是 token/draft 单位的平均接受长度。本文所有“接受率提升”若无特殊说明，均指这个指标。
...
DFlash 是独立于 verifier 的 draft 模块。它根据当前上下文和 verifier 隐状态，一次性并行预测一整块候选 token，例如 spec=7 时预测 7 个 token。随后 verifier 对这 7 个 token 做并行校验，并从第一个 token 开始接受连续匹配的前缀。
...
DFlash 的问题也很直接: **开源 DFlash draft 主要缺少多模态分布**。文本场景中的 draft 不一定知道图像条件会如何改变 verifier 的输出，因此在 ALLaVA 这类图文输入上，原版 DFlash 的接受长度明显偏低。

### 2.2 MTP: verifier 原生多 token 预测头

![MTP algorithm](assets/mtp_algorithm.svg)

MTP(Multi-Token Prediction)不是独立 draft，而是 verifier 原生权重里自带的多 token 预测头。它利用 verifier 的隐藏状态逐步预测后续 token。因为 MTP 头与 verifier 表示更贴近，通常可以获得更高的接受长度。

MTP 的优势是 **接受率上限高**，尤其在 122B 这种 verifier 成本很高的场景中，高接受长度会更直接地转化为吞吐。它的代价是 draft 侧相对更重，而且原生头本身已经比较强，进一步提升的空间没有 DFlash 那么大。

## 3. 研究故事: 先补 DFlash 的多模态短板，再迁移到 MTP

这轮实验不是一开始就直接冲 MTP，而是沿着一个很自然的问题展开:
...
最终结果与这个假设一致: **DFlash 相对提升最大，MTP 绝对结果最高。**
...
| mean accept/draft | token/draft | 每轮投机平均接受 token 数，本文主接受率指标 |
...
最早的 DFlash 训练并不顺利。KL loss 虽然能降低分布距离，但投机解码在 greedy 场景只关心 top-1 是否一致，因此 KL 并没有有效提升验收。切换到 CE loss 后，draft 直接学习 verifier 的 top-1 token，接受长度开始上升。

第二个关键点是精度。纯 bf16 下，AdamW 的小更新容易被舍入吞掉，first-position acceptance 长期不动。改成 fp32 master weight 并把 LR 提到 3e-5 后，first-position 第一次明显上升。
...
这说明 DFlash 的问题确实主要来自多模态分布不匹配，自蒸馏能把这个缺口补回来。不过在 122B 上，trained DFlash 仍低于 MTP，说明大模型 verifier 更受益于 MTP 的高接受长度。

## 6. 把同样思路迁移到 MTP

DFlash 验证了“用多模态自蒸馏对齐 verifier greedy 输出”是有效的。下一步自然是把同样思路迁移到 MTP: 不再训练独立 draft，而是在 verifier 原生 MTP 头上继续微调。

MTP 的部署需要一个额外工程步骤: vLLM 无法直接挂裸 MTP checkpoint，因此需要把微调后的 MTP 层 stitch 回完整 verifier 权重，然后再 serve。训练精度上，MTP 使用 bf16；此前验证过 fp32 会破坏其 lm_head 行为。
...
纯微调把 MTP 特化到了 ALLaVA，域内收益明确，但 MMStar 上有轻微回退。这个回退不是能力崩溃，因为 first-position 几乎不变；损失主要发生在更深位置。
...
权重插值几乎完整保留了 ALLaVA 域内收益，同时把 MMStar 从负收益修复为正收益。这说明 MTP 的域内增量与原生头的通用能力可以在参数空间里兼容。

### 6.3 122B MTP: `beta=0.6` 与 `alpha=0.5` 的组合

122B MTP 使用自身蒸馏的 ALLaVA 50k 数据。由于投机接受是链式的，早位 token 的价值远高于深位 token: 深位只有在前面所有 token 都被接受后才会生效。因此我们做了 step weight A/B:
...
`beta=0.6` 在整体 mean accept 上优于 `beta=1.0`，域外回退也更小。这说明“把权重平均分给深位”并不划算；深位确实重要，但早位是链式接受的入口，损失早位会让后面的位置没有机会生效。

随后使用 `alpha=0.5` 做 MTP soup，获得更适合通用服务的结果:
...
到这里，9B 和 122B 的 MTP 结论一致: 纯微调能提高域内接受率，权重插值能把域外从回退修复为正收益。

## 7. 为什么 DFlash 提升更大，而 MTP 结果更强

### 7.1 DFlash 的收益来自补齐多模态数据分布

original DFlash 的短板不是结构完全不可用，而是训练分布与多模态 verifier 的解码分布不匹配。自蒸馏数据直接给它补上了图文条件下的 target token，因此 DFlash 的相对提升最大:

- 9B ALLaVA: mean accept **+37.0%**，QPS **+20.2%**。
- 122B ALLaVA: mean accept **+30.2%**，QPS **+16.0%**。

这也是为什么 DFlash 的故事最清楚: 开源 draft 没有多模态数据，我们用多模态自蒸馏补它的分布缺口，然后接受率显著上升。
...
因此 MTP 的核心不是“从弱变强”，而是“在强基线上继续挖域内收益，并用权重插值守住域外”。

### 7.3 早位 token 是所有训练策略的关键

投机接受是前缀链式接受。第 1 个 token 错了，后面即使全对也不会被接受。因此:
...
机制上 self-forcing 确实会把接受分布向深位移动，但它损失了早位。由于早位在链式接受中杠杆最高，深位收益没能补回早位损失。本方向暂缓；若后续重启，需要固定 `beta=0.6`，用更小比例的 self-forcing，并从 0 ramp up。
...
这些工程改动让后续可以按“改训练策略、内网 GPU 验证、回传 log、汇总结论”的方式迭代。

## 10. 局限与下一步

### 10.1 局限

1. 当前主实验每个数据集为 128 prompts。接受率指标较稳定，但 QPS 仍有单次服务波动。
2. 部分配置没有完整 QPS，例如 9B `alpha=0.5` MTP soup 记录了接受长度，但没有单独记录 QPS。
3. 离线评测不等价于高并发在线服务。DFlash 在高并发场景下可能更有优势，需要在线 workload 复核。
4. DFlash 122B 虽然显著超过 original DFlash，但仍落后 MTP。

### 10.2 下一步

1. **MTP**: 通用服务优先使用 `alpha=0.5` soup；继续扫 `alpha in {0.3,0.4,0.5,0.6,0.7,1.0}` 与 serve-time `spec in {3,5,7}`。
2. **DFlash**: 做 per-epoch acceptance sweep，用真实接受率选 checkpoint；同时对 first-position 加权。
3. **高并发 D-Cut**: 当 draft 已经足够便宜时，瓶颈会转向 verifier 校验。D-Cut 的目标是减少每轮 verify token 数，例如 spec=7 时平均只校验 4 个 token，verify token 约减少 **43%**。当前 GPU 路径已跑通，下一步迁移至 NPU。
4. **Domino-style 与 causal DFlash**: 这些方向与 D-Cut、MTP 微调基本正交，可作为后续提升 draft 质量或减少冗余计算的候选方向；目前不计入已验证主结论。
...
MTP 路线需先完成 head stitch，再以同一 verifier、同一 spec、同一 validation tail 对 original 与 trained/soup 进行 A/B 评测。
